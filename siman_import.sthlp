...
{title:Title}
{phang}
{bf:siman_import} {hline 2} Import performance measures data for use in siman
...
{synoptline}
{syntab:Required }
{synopt:{opt perf(varname)}}String variable containing the names of the performance 
measure. The performance measure names must be taken from those listed in {help simsum##pm_options:help simsum}.{p_end}
{synopt:{opt estimate(varname)}}Numeric variable containing the values of the performance measures.{p_end}
{syntab:Optional}
{synopt:{opt dgm(varlist)}}Variable(s) defining the data generating mechanism. {p_end}
{synopt:{opt target(varname)}}Variable defining the target or estimand. {p_end}
{synopt:{opt method(varname)}}Variable defining the method of analysis.{p_end}
{synopt:{opt se(varname)}}Numeric variable containing the Monte Carlo standard errors of the performance measures.{p_end}
{synopt:{opt true(varname)}}Numeric variable containing the true values.{p_end}
{synoptline}
{p2colreset}{...}
{p 4 6 2}
...
{pstd}{cmd:siman_import} is a utility to import a dataset of performance estimates that have already 
...
requires some data reformatting to get the data into the long-long format required by {cmd:siman_import}.
...
{phang}. {stata "siman_import, dgm(theta tau2 k pc rho) target(target) method(method) estimate(est) perf(_perfmeascode)"}{p_end}

{pstd}Now use the {cmd:siman} suite
