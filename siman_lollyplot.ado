...
*   version 0.13.2  15dec2023
*   version 0.13.2  15dec2023   IW 'if' is a condition not an option
*   version 0.13.1  25oct2023   IW clearer error if no obs; helpful message with pause option
*   version 0.13    13sep2023     IW add level() option; streamline pm variables; show PMs in order requested; use pstyle to harmonise graph; new logit option calculates CI for power & coverage on logit scale 
*   version 0.12.3  22aug2023   IW bug fix with name("n")
*   version 0.12.2  22aug2023   IW handles target = numeric or string 
*   version 0.12.1  17aug2023   IW 
*   version 0.12    14aug2023   IW changed to fast graph without graph combine; works for one or multiple dgm. NB gr() no longer valid.
*   version 0.11    05may2023   IW add "DGM=" to subtitles and "method" as legend title
*   version 0.10    08mar2023    
...
*   version 0.9     23dec2022   IW added labformat() option; changed to use standard twoway graph with standard legend
*   version 0.8     05dec2022   TM added 'rows(1)' so that dgms all appear on 1 row.
*   version 0.7     14nov2022   EMZ added bygraphoptions().
*   version 0.6     05sep2022   EMZ bug fix to allow if target == "x".
*   version 0.5     14jul2022   EMZ fixed bug to allow name() in call. 
*   version 0.4     11jul2022   EMZ changed pm and perfeascode to _pm and _perfmeascode.
*   version 0.3     16may2022   EMZ bug fixing with graphical displays.  Added in graphoptions() for constituent graph options.
*   version 0.2     24mar2022   EMZ further updates from IW testing.
*   version 0.1     10jan2021   EMZ updates from IW testing (bug fix).
*   version 0.0     09dec2020   Ella Marley-Zagar, MRC Clinical Trials Unit at UCL. Based on Tim Morris' simulation tutorial do file.
* File to produce lollyplots
