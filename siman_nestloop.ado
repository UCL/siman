...
*   version 0.8.2    17aug2023   IW renamed descriptor graph options from frac* and legend* to dg*; new checks for wrong dgmorder(), only 1 dgm; handle missing or numeric target; remove some unwanted `if's; handle method as any type
*   version 0.8.1    16aug2023   IW changed how main graph is written, so that stagger works; correct looping over PMs and targets; general tidying up and clarifying; correct use of true
*   version 0.8      14aug2023   IW extended lines to include last scenario; reduced default PMs; range correctly allows for all methods
*   version 0.7.3    01aug2023   IW added legendoff option; made name() work
*   version 0.7.2    22may2023   IW fixes
*   version 0.7.1    13mar2023   EMZ added error message
*   version 0.7      11aug2022   EMZ fixed bug to allow name() in call  
*   version 0.6      11jul2022   EMZ renamed created variables to have _ infront
*   version 0.5      19may2022   EMZ added error message
*   version 0.4      31mar2022   EMZ minor updates
*   version 0.3      10jan2022   EMZ updates from IW testing.
*   version 0.2      06dec2021   Numeric dgm variable labels to 2d.p. in graph. dgm() in nestloop to take the order specified in dgmorder() so that 
*                                siman_setup does not need to be re-run if the user would like to change the order of the dgms.
*   version 0.1      02dec2021   Ella Marley-Zagar, MRC Clinical Trials Unit at UCL. Based on Ian White's nplot.ado
